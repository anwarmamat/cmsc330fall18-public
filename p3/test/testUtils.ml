...
let string_of_int_list_list l = Printf.sprintf "[%s]" @@ String.concat "; " @@ List.map string_of_int_list l;;
...
let assert_set_set_eq lst1 lst2 =
  let es = (fun l -> List.sort_uniq compare (List.map (List.sort compare) l)) in
  assert_equal (es lst1) (es lst2)

let assert_trans_eq lst1 lst2 =
  let es = (fun l -> List.sort_uniq compare (List.map (fun (l1, t, l2) -> (List.sort compare l1, t, List.sort compare l2)) l)) in
  assert_equal (es lst1) (es lst2)
