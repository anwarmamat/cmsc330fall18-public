...
let string_of_vtx show lst = List.fold_left (fun acc (v, f) ->
  let shape = if f then "doublecircle" else "circle" in
  acc ^ (Printf.sprintf "%d [label=\"%s\",shape=%s];\n" (Hashtbl.hash v) v shape)) "" lst
let string_of_ed show lst = List.fold_left (fun acc ((s1, _), c, p, (s2, _)) ->
  acc ^ (Printf.sprintf "%d -> %d [label=\"%s\"];\n"
          (Hashtbl.hash s1) (Hashtbl.hash s2) c)) "" lst
...
if line2 = "n" then
  ...
  if write_nfa_to_graphviz string_of_int_list (nfa_to_dfa nfa) then
